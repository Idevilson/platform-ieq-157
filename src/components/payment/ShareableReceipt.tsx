...
export function ShareableReceipt({
  ...
  const handleShareWhatsApp = () => {
    const brindeLine = hasBrinde ? '\n🎁 Ganhei a pulseira de brinde!' : ''
    const text = `🎉 Estou confirmado(a) no *${eventName}*!${brindeLine}\n\n📅 ${eventDates}\n📍 ${eventLocation}\n\n_Faça sua inscrição também!_`
        ...
        <div className="relative z-10 flex flex-col justify-between h-full p-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Image src="/images/only-logo.png" alt="IEQ" width={32} height={32} className="opacity-90" />
              <div className="text-left">
                <p className="text-gold text-sm font-bold">IEQ SEDE CAMPO 157</p>
                <p className="text-gold/60 text-[10px] uppercase tracking-wide">Redenção - PA</p>
              </div>
            </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center py-4">
            {eventLogoUrl ? (
              <div className="mb-3">
                <Image src={eventLogoUrl} alt={eventName} width={200} height={80} className="object-contain max-h-[80px]" />
              </div>
            ) : (
            ...
            {eventSubtitle && (
              <p className="text-gold/70 text-sm mb-4">{eventSubtitle}</p>
            )}
            <h2 className="text-2xl font-bold text-white leading-tight mb-2">
              {participantName}
            </h2>
            {hasBrinde && (
              <div className="mt-2 px-4 py-1.5 bg-gold/20 border border-gold/40 rounded-full">
                <span className="text-gold text-xs font-semibold">🎁 Pulseira garantida</span>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between gap-4 pt-3 border-t border-gold/20">
            ...
            <div className="bg-white p-1.5 rounded-lg shrink-0">
              <QRCodeSVG value={inscriptionId} size={56} level="M" bgColor="#ffffff" fgColor="#1a1a2e" />
            </div>
          </div>
        </div>
      </div>
