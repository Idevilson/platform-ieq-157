...
export function InscriptionLookup({
  ...
  useEffect(() => {
    if (autoSearch && initialCpf && !hasAutoSearched) {
  ...
  }, [autoSearch, initialCpf, hasAutoSearched, lookupMutation]);
  ...
  const handleTogglePayment = (inscriptionId: string) => {
    setExpandedPayment(
      expandedPayment === inscriptionId ? null : inscriptionId,
    );
    setCopiedPix(false);
  };
  ...
  const isWaitingForResults =
    autoSearch && hasAutoSearched && results === null && !error;

  return (
    <div
      className={`bg-bg-secondary rounded-2xl border border-gold/10 ${compact ? "p-4" : "p-6"}`}
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <h4
            className={`font-semibold text-gold ${compact ? "text-base mb-2" : "text-lg mb-1"}`}
          >
            Consultar Inscrição
          </h4>
          <p className="text-sm text-text-secondary">
            Digite seu CPF para consultar suas inscrições
            {eventId ? " neste evento" : " em todos os eventos"}.
          </p>
        </div>
      ...
      {(isLoading || isWaitingForResults) && (
        <div className="mt-6 flex flex-col items-center justify-center py-8">
          <div className="w-8 h-8 border-3 border-gold/30 border-t-gold rounded-full animate-spin mb-3" />
          <p className="text-text-secondary text-sm">Buscando inscrições...</p>
        </div>
      )}
              ...
              <div className="space-y-3">
                {results.map((result) => (
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {result.inscription.paymentStatus && (
                          <span
                            className={`inline-block px-3 py-1 text-xs font-medium rounded-full border ${getPaymentStatusClass(result.inscription.paymentStatus)}`}
                          >
                            {
                    ...
                    <div className="text-sm text-text-secondary space-y-1">
                      {result.inscription.guestData && (
                        <p>
                          ...
                          {result.inscription.guestData.nome}
                        </p>
                      )}
                      <p>
                    ...
                    {!result.payment &&
                      result.inscription.status === "pendente" && (
