...
export default function AdminEventDetailPage() {
  ...
  const normalizeCity = (cidade?: string) => cidade?.trim().toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '')
  const isRedencao = (cidade?: string) => normalizeCity(cidade) === 'redencao'
  const insideCount =
    inscriptions.filter(i => isRedencao(i.cidade)).length +
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <StatCard
          label="De Redenção"
          value={insideCount}
          subValue={totalInscriptions > 0 ? `${Math.round((insideCount / totalInscriptions) * 100)}% do total` : undefined}
          icon={
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <StatCard
          label="De Fora"
          value={outsideCount}
          subValue={totalInscriptions > 0 ? `${Math.round((outsideCount / totalInscriptions) * 100)}% do total` : undefined}
          icon={
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
