...
function BatchInscricaoContent({ id }: { id: string }) {
  const router = useRouter()
  const { user: authUser, loading: authLoading } = useAuth()
  const { data: evento, isLoading: eventLoading, error } = useEventById(id)
  const { data: categorias, isLoading: categoriasLoading, error: categoriasError, refetch: refetchCategorias } = useEventCategories(id)
  const [cpfInput, setCpfInput] = useState<string>()
  const [showForm, setShowForm] = useState(false)

  const { data: myBatches, isLoading: myBatchesLoading } = useMyBatches(id, !!authUser)
  const { data: lookedUpBatches, isLoading: lookupLoading } = useBatchLookup(cpfInput, id)
      ...
      <section className="py-12">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <div className="card py-12">
            ...
            <p className="text-text-secondary mb-6">As inscrições para este evento não estão disponíveis.</p>
    ...
    return (
      <section className="py-12">
        <div className="max-w-2xl mx-auto px-4">
          <div className="mb-8">
            {backLink}
            <h1 className="text-2xl font-bold text-text-primary">{evento.titulo}</h1>
          ...
          {!lookupLoading && lookedUpBatches?.map(batch => (
            <div key={batch.batchId} className="mt-4">
              <BatchInscriptionCard
                batch={batch}
                onViewPayment={() => router.push(`/eventos/${id}/inscricao-coletiva/confirmado?batchId=${batch.batchId}`)}
              />
            </div>
          ))}
  ...
  const eventCategorias = categorias?.length ? categorias : (evento.categorias ?? [])

  if (categoriasError || !eventCategorias.length) {
    return (
      <section className="py-12">
        <div className="max-w-2xl mx-auto px-4">
  ...
  const mappedCategories = eventCategorias.map((c: {
...
export default function BatchInscricaoPage({ params }: Props) {
  const { id } = use(params)
  return <BatchInscricaoContent id={id} />
}
